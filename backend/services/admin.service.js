...
export const createSubCategory = async (data) => {
    if (!data.name || !data.category || !data.image) {
        throw new Error("Name, Category ID, and Image are required to create a sub-category");
    }
...
export const addProduct = async (data) => {
    ...
    data.finalPrice = Math.floor(data.price - (data.discount || 0));
...
export const createCupon = async (data) => {
    ...
    if (!code || !discount || !expirationDate || !usageLimit) {
        throw new Error("Code, Discount, Expiration Date, and Usage Limit are required to create a coupon");
    }
