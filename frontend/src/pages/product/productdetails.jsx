...
import { useGetProductByIdQuery } from "../../redux/Admin/userAPI";
...
export default function ProductDetails() {
  ...
  // product is inside data.data per your API
  const product = data?.data || null;

  // context functions (optional — fallback safe)
  const {
    addToCart = () => {},
    url = "",
  ...
  // when product arrives, set defaults
  useEffect(() => {
    if (product) {
      setMainIndex(0);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white p-6">
        <div className="text-center">
          ...
          <Link to="/shop" className="text-indigo-300 underline mt-4 inline-block">
            Back to shop
          </Link>
        </div>
      </div>
              ...
              {product.images?.map((img, i) => (
                <button
                  ...
                  className={`w-20 h-20 rounded-xl overflow-hidden border ${
                    i === mainIndex ? "border-indigo-400 scale-105" : "border-white/10"
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md shadow-[0_30px_80px_rgba(0,0,0,0.6)]">
                ...
                <div className="flex items-center">
                  {Array.from({ length: 5 }).map((_, idx) => (
                    <Star
                      key={idx}
                      size={16}
                      className={idx < ratingRounded ? "text-indigo-400" : "text-white/20"}
                    />
                  ))}
                </div>
            ...
            <div className="mt-6 flex items-center gap-4">
              <div>
                <p className="text-indigo-300 font-bold text-2xl">{format(product.finalPrice)}</p>
                {product.discount > 0 && (
                  <p className="text-white/40 line-through text-sm">₹{product.price}</p>
                )}
              </div>

              <div className="ml-auto text-sm text-white/60">
                <p>Stock: <span className="text-white">{product.stock}</span></p>
                <p className="mt-1">Discount: <span className="text-white">{product.discount}%</span></p>
              </div>
            </div>
                  ...
                  {product.sizes?.map((s) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl border ${
                        selectedSize === s ? "border-indigo-400 bg-indigo-400/10" : "border-white/10"
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3 items-stretch">
              <div className="flex items-center gap-3 bg-white/3 border border-white/8 rounded-xl px-3 py-2">
                ...
                <div className="px-4 py-1 bg-transparent text-white font-semibold">{qty}</div>
                <button
              ...
              {/* Add to cart */}
              <motion.button
                whileHover={{ scale: 1.02, boxShadow: "0 12px 40px rgba(99,102,241,0.18)" }}
                whileTap={{ scale: 0.98 }}
                onClick={() => {
              ...
              {/* Buy now */}
              <motion.button
                whileHover={{ scale: 1.02, boxShadow: "0 12px 40px rgba(99,102,241,0.12)" }}
                whileTap={{ scale: 0.98 }}
                onClick={() => {
                  // send user to checkout with product id/qty (adjust as your app expects)
                  navigate("/checkout", { state: { productId: product._id, qty, selectedSize } });
                }}
            ...
            <div className="mt-6 text-sm text-white/60 grid grid-cols-2 gap-2">
              <div>Category: <Link to={`/category/${product.category?._id}`} className="text-indigo-300 underline ml-1">{product.category?.name}</Link></div>
              <div>Subcategory: <span className="text-white ml-1">{product.subCategory?.name}</span></div>
              <div>Rating: <span className="text-white ml-1">{product.rating}</span></div>
              <div>SKU: <span className="text-white ml-1">{product._id.slice(-8)}</span></div>
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md">
            ...
            <p className="text-white/60 mt-3 leading-relaxed">{product.description}</p>
